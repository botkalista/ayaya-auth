...
exports.needAuthentication = exports.createRouter = void 0;
...
function needAuthentication(req, res, next) {
    return __awaiter(this, void 0, void 0, function () {
        var authorization, token, data, user;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    authorization = req.headers.authorization;
                    if (!authorization)
                        return [2 /*return*/, res.status(401).json({ error: 'No auth' })];
                    token = authorization.replace('Bearer ', '');
                    data = repo.verifyToken(token);
                    if (!data)
                        return [2 /*return*/, res.status(401).json({ error: 'Invalid token' })];
                    return [4 /*yield*/, repo.getUserByUid(data.uid)];
                case 1:
                    user = _a.sent();
                    if (!user)
                        return [2 /*return*/, res.status(401).json({ error: 'Auth error' })];
